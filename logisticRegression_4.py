...
class LogisticRegression(torch.nn.Module):
    ...
    def predict_temp_soft_labels(self, feat):
        return torch.sigmoid(self.linear(feat) / temperature)

    def score(self, feat, labels):
        y_prob = torch.sigmoid(self.linear(feat))
...
def fit(step, feat, labels, train_mask, val_mask, y_val, prev_model):
    ...
    for i in range(epoch):
        for j in range(0, int(1. / batch_prop) + 1):
            ...
            y_train_prob = model.forward(X_train_batch)
            y_pseudo_prob = model.forward(X_pseudo_batch)
            num_train = y_train_batch.shape[0]
            num_pseudo = y_pseudo_batch.shape[0]
            if step == 0:
                binary_entropy_train_loss = F.binary_cross_entropy(y_train_prob, y_train_batch)
            else:
                binary_entropy_train_loss = F.binary_cross_entropy(y_train_prob,
                                                              y_train_batch) + alpha * F.binary_cross_entropy(
                    y_pseudo_prob, y_pseudo_batch) + beta * (
                                               y_pseudo_prob * torch.log(y_pseudo_prob)).sum() / (
                                               num_pseudo * num_classes)
            binary_entropy_train_loss.backward()
            optimizer.step()

        if count == 0:
            best_model = copy.deepcopy(model)
        y_val_prob = model.forward(X_val)
        binary_entropy_val_loss = F.binary_cross_entropy(y_val_prob, y_val)
        # accuracy_val = accuracy(y_val_prob, y_val)
        if binary_entropy_val_loss - prev_loss_val > 0 or prev_loss_val - binary_entropy_val_loss < 1e-2:
            count += 1
        else:
            count = 0
        if count == early_stopping:
            break
        prev_loss_val = binary_entropy_val_loss
