    ...
    for i in range(epoch):
        for j in range(0, int(1. / batch_prop) + 1):
            ...
            if step == 0:
                ...
                # pseudo-label certainty
                pseudo_entropy = - (torch.exp(y_pseudo_log_prob.data) * y_pseudo_log_prob.data).sum(1)
                pseudo_weight = 1 - pseudo_entropy / np.log(num_classes)
                pseudo_weight = pseudo_weight.reshape(-1, 1)

                # class imbalance
                y_train_cls = y_train_log_prob.data.max(1)[1]
                y_true_train_cls = y_train_batch.max(1)[1]
                y_pseudo_cls = y_pseudo_log_prob.data.max(1)[1]

                cls_imb_weight = np.array([np.reciprocal(float(
                    torch.where(y_true_train_cls == i)[0].shape[0] + torch.where(y_pseudo_cls == i)[0].shape[0])) for i in
                    range(num_classes)])
                y_train_cls_weight = [cls_imb_weight[i] for i in y_train_cls]
                y_pseudo_cls_weight = [cls_imb_weight[i] for i in y_pseudo_cls]
                y_train_cls_weight = torch.FloatTensor(y_train_cls_weight).reshape(-1, 1).to(y_train_cls.device)
                y_pseudo_cls_weight = torch.FloatTensor(y_pseudo_cls_weight).reshape(-1, 1).to(y_train_cls.device)

                entropy_train = (y_train_cls_weight * y_train_batch * y_train_log_prob).sum() / num_train + (
                        pseudo_weight * y_pseudo_cls_weight * y_pseudo_batch * y_pseudo_log_prob).sum() / (
                                        num_pseudo * num_classes) \
                                + (pseudo_weight * y_pseudo_cls_weight * torch.exp(
                    y_pseudo_log_prob) * y_pseudo_log_prob).sum() / (
                                        num_pseudo * num_classes)
