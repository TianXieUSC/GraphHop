...
def load_ppi_data(path, original_split=True, training_ratio=0.1):
    if os.path.exists(path + 'processed/'):
        ...
        G, feats, id_map, class_map = load_data(path + 'ppi')
        path += 'processed/'
        os.makedirs(path)
    ...
    multi_hot_labels = np.array([class_map[i] for i in id_order])
    ...
    if original_split:
        ...
        train_mask = np.zeros(multi_hot_labels.shape[0], dtype=bool)
        val_mask = np.zeros(multi_hot_labels.shape[0], dtype=bool)
        test_mask = np.zeros(multi_hot_labels.shape[0], dtype=bool)

        training_index = int(train_mask.shape[0] * training_ratio)
        train_mask[:training_index] = True
        val_mask[training_index:(training_index + 5000)] = True
        test_mask[training_index + 5000:] = True

    # filter out the same nodes in adjacency matrix
    feats = feats[node_order]
    print('PPI data loaded.')
    return feats, multi_hot_labels, adj, train_mask, val_mask, test_mask


if __name__ == '__main__':
    path = "/Users/tian/Documents/P8_Graph_Based_Learning/datasets/molecular/ppi/"
    feats, one_hot_labels, adj, train_mask, val_mask, test_mask = load_ppi_data(path, True)
