...
from logisticRegression_4 import fit
...
from utils import edge_weight, multiHops, normalize, one_shot_edge_weight, random_walk_normalize, pure_k_hops, \
    sparse_mx_to_torch_sparse_tensor, accuracy_multilabel
...
for i in range(epoch + 1):
    ...
    train_score = accuracy_multilabel(new_feat[train_mask], one_hot_labels[train_mask])
    val_score = accuracy_multilabel(new_feat[val_mask], one_hot_labels[val_mask])
    test_score = accuracy_multilabel(new_feat[test_mask], one_hot_labels[test_mask])
